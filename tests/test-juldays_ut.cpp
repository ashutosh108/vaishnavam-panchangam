...
TEST_CASE ("construct from Y, m, d") {
    std::stringstream s;
    s << JulDays_UT{2019_y, March, 10_d};
    REQUIRE(s.str() == "2019-03-10 00:00:00.000000 UTC");
}

TEST_CASE ("construct from Y, m, d, hours") {
    std::stringstream s;
    s << JulDays_UT{2019_y, March, 10_d, double_hours{5.25}};
    REQUIRE(s.str() == "2019-03-10 05:15:00.000000 UTC");
}

TEST_CASE("hours matter") {
    JulDays_UT t1{2019_y, March, 10_d, double_hours{0.0}};
    JulDays_UT t2{2019_y, March, 10_d, double_hours{6.0}};
...
TEST_CASE("create from Y M D h m s", "[hms]") {
    JulDays_UT t1{2019_y, March, 17_d, double_hours{4.2267416752777778}};
    JulDays_UT t2{2019_y, March, 17_d, 4, 13, 36.270031};
    REQUIRE(t1 == t2);
}
...
TEST_CASE("Can create from Date") {
    ...
    JulDays_UT t{d};
    REQUIRE(t == JulDays_UT{2019_y, March, 9_d});
}

TEST_CASE("Can add double to JulDays_UT") {
    // make sure that first argument to + can be const
    JulDays_UT const t1{2019_y, March, 17_d};
    REQUIRE(t1+double_days{1.0} == JulDays_UT{2019_y, March, 18_d});
}

TEST_CASE("+= returns correct value") {
    REQUIRE((JulDays_UT{2019_y, March, 17_d}+=double_days{1.0}) == JulDays_UT{2019_y, March, 18_d});
}

TEST_CASE("+= modifies JulDays_UT") {
    JulDays_UT t{2019_y, March, 17_d};
    t += double_days{1.0};
    REQUIRE(t == JulDays_UT{2019_y, March, 18_d});
}

TEST_CASE("Substracting double from JulDays_UT means substracting days") {
    JulDays_UT const t1{2019_y, March, 17_d};
    REQUIRE(t1-double_days{1.0} == JulDays_UT{2019_y, March, 16_d});
}

TEST_CASE("-= returns correct value") {
    REQUIRE((JulDays_UT{2019_y, March, 17_d}-=double_days{1.0}) == JulDays_UT{2019_y, March, 16_d});
}

TEST_CASE("-= modifies JulDays_UT") {
    JulDays_UT t{2019_y, March, 17_d};
    t -= double_days{1.0};
    REQUIRE(t == JulDays_UT{2019_y, March, 16_d});
}

TEST_CASE("Can compare <") {
    REQUIRE(JulDays_UT{2019_y, March, 17_d} < JulDays_UT{2019_y, March, 18_d});
}

TEST_CASE("Can compare >") {
    REQUIRE(JulDays_UT{2019_y, March, 18_d} > JulDays_UT{2019_y, March, 17_d});
}

TEST_CASE("Can substract JulDays_UT from JulDays_UT") {
    REQUIRE((JulDays_UT{2019_y, April, 16_d} - JulDays_UT{2019_y, April, 10_d}) == double_days{6.0});
}
