 ...
 */
std::string detect_ekadashi([[maybe_unused]] const std::string & text) {
    // TODO: implement
    return "";
}
 ...
 */
std::size_t check_vrata(const std::vector<std::string> & row, std::size_t col) {
    std::string ekadashi_name = detect_ekadashi(row[col]);
    if (ekadashi_name.empty()) return 0;
    // first +1 to compensate zero-based counting
    // second +1 means we require that there is at least one more column.
    REQUIRE(row.size() >= col+1+1);
    std::size_t cells_handled = check_ekadashi(row, col, ekadashi_name);
    REQUIRE(cells_handled >= 2);
 ...
 */
std::size_t check_vratas_from_row(const std::vector<std::string> & row, const col_to_date & date_map) {
    ...
    for (auto iter = date_map.begin(); iter != date_map.end(); ++iter) {
        ...
        std::size_t handled_cells = check_vrata(row, col);
...
void test_one_precalculated_case(const std::string & slug) {
    ...
    for (size_t row=1; row < row_count; ++row) {
        ...
        std::size_t ekadashi_etc_count = check_vratas_from_row(row_data, date_headers);
        std::string row_str;
        bool first = true;
