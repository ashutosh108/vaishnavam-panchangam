...
TEST_CASE("get sunrise") {
    ...
    auto sunrise = Swe{c}.get_sunrise(JulDays_UT{2019_y, March, 10_d});
...
TEST_CASE("get_sunset") {
    Location c{50.45, 30.523333};
    auto sunset = Swe{c}.get_sunset(JulDays_UT{2019_y, March, 10_d});
    REQUIRE(sunset.has_value());
    REQUIRE(*sunset == JulDays_UT{2019_y, March, 10_d, 15, 48, 33.812600});
}

TEST_CASE("get sun longitude") {
    double sun_longitude = Swe{arbitrary_coord}.get_sun_longitude(JulDays_UT{2019_y, March, 10_d});
    REQUIRE(sun_longitude == Approx(349.1222311334));
}

TEST_CASE("get moon longitude") {
    double moon_longitude = Swe{arbitrary_coord}.get_moon_longitude(JulDays_UT{2019_y, March, 10_d});
    REQUIRE(moon_longitude == Approx(26.2874840949));
}

TEST_CASE("get tithi") {
    JulDays_UT t2{2019_y, March, 21_d, double_hours{1.716666}}; // around 1:43am (UTC time), peak of purnima
